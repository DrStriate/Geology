...
class YhsPath:
  def __init__(self, parent):
    ...
    self.NAPAdist = PAvel(NA_plate_azimuth, NA_plate_speed)
  ...
  def setupEulerPoles (self, useGpsData):
    ...
    if useGpsData:
      ...
      pnw_v_dist = PVel(0.0, 5.0)

    self.pnw_v_azdist = gh.getPAvel(pnw_v_dist.east, pnw_v_dist.north)
  ...
  def getRotPoleAndVelocity(self, raw_data_center, distance):
    ...
    return rot_pole, PVel(gn_out['t_x'] / SF, gn_out['t_y'] / SF)
  ...
  def display_rot_pole_info(self):
    ...
    label_text2 = f"az: {self.pnw_v_azdist.azimuth:.2f} deg, v: {(self.pnw_v_azdist.vel):.2f} km/ma"
  ...
  def get_yhs_loc(self, yrs): # yrs is years
    ...
    paDistNA = gh.PAvel(self.NAPAdist.azimuth, self.NAPAdist.vel)
    pole_NA = ek.getEulerPoleFromPlocAndPazvel(self.yhs_loc, paDistNA)
    ...
    self.pnw_v_pole = ek.getEulerPoleFromPlocAndPazvel(loc_1, self.pnw_v_azdist)
    ...
    if self.pole_model == 1: # move YHS loc by pnw_v_pole then rotate (most direct model)
      ...
      new_rot_pole_ploc, disp = ek.getPoleRotationOfPoint(self.pnw_v_pole, self.pnw_rot_pole.ploc(), yrs / 1e6)
